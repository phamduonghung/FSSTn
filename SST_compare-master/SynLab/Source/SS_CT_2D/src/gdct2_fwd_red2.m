function [C C1 C2] = gdct_fwd_red2(x, is_real,sz,R_high,R_low,  rad,is_cos, t_sc, s_sc, red,t2, wedge_length_coarse)
% Note: This code is only used to test the robustness of the 2D SST. t2 is
% a spatial parameter to indicating a spatial position used to generate the
% figures in the paper "Robustness Analysis of Synchrosqueezed Transforms".
%
% gdct_fwd.m - Generalized Discrete Curvelet Transform
%
% Inputs
%   x           N1-by-N2 matrix
%
...
tempC = fftshift(ifft2(ifftshift(temp)))*sqrt(sz(1)*sz(2))/fac;
C{1}{1} = tempC(:,t2);
temp = zeros(sz);
